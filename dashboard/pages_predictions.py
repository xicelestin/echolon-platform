...
def render_predictions_page(data=None, kpis=None, format_currency=None, format_percentage=None, format_multiplier=None):
    ...
    # Always show forecast by default (no click required)
    ...
    st.markdown("""<div style='margin-top:30px; margin-bottom:20px'><h3 style='font-size:18px; font-weight:600;'>💡 Insights for Your Business</h3></div>""", unsafe_allow_html=True)
    
    pred_metrics = calculate_key_metrics(data)
    industry = st.session_state.get('industry', 'ecommerce')
    pred_insights = generate_personalized_insights(data, pred_metrics, industry)
    
    positive = [i for i in pred_insights if i['type'] == 'positive']
    needs_attention = [i for i in pred_insights if i['type'] in ('opportunity', 'critical')]
    ...
    with col1:
        ...
        if positive:
            for i in positive[:3]:
                st.success(f"{i['icon']} {i['title']}")
        else:
            st.info("Keep optimizing — your forecast suggests stable trends. Focus on top channels.")
    
    with col2:
        st.markdown("**⚠️ Areas to Monitor**")
        if needs_attention:
            for i in needs_attention[:3]:
                st.warning(f"{i['icon']} {i['title']} — {i['action'][:80]}...")
        else:
            st.info("No major concerns. Continue monitoring KPIs and forecast accuracy.")
