...
def generate_investor_report_pdf(
    ...
    story.append(Paragraph(f"{company_name} — Business Intelligence Report", title_style))
...
def generate_whatif_scenario_pdf(
    scenarios: Dict[str, Any],
    params: Dict[str, Any],
    company_name: str = "Your Business"
) -> Optional[bytes]:
    """Generate PDF for What-If scenario comparison. For saving Best Case as target."""
    if not REPORTLAB_AVAILABLE:
        return None
    buffer = io.BytesIO()
    doc = SimpleDocTemplate(buffer, pagesize=letter, rightMargin=0.75*inch, leftMargin=0.75*inch)
    styles = getSampleStyleSheet()
    story = []
    title_style = ParagraphStyle('CustomTitle', parent=styles['Heading1'], fontSize=22, spaceAfter=6, alignment=TA_CENTER)
    story.append(Paragraph(f"What-If Scenario — {company_name}", title_style))
    story.append(Paragraph(f"Generated {datetime.now().strftime('%B %d, %Y')}", styles['Normal']))
    story.append(Spacer(1, 0.3*inch))
    ...
    metrics_data = [
        ['Scenario', '12-Month Revenue', 'Growth %'],
        ['Best Case', f"${best.get('revenue', 0):,.0f}", f"+{best.get('growth', 0):.1f}%"],
        ['Expected', f"${expected.get('revenue', 0):,.0f}", f"+{expected.get('growth', 0):.1f}%"],
        ['Worst Case', f"${worst.get('revenue', 0):,.0f}", f"+{worst.get('growth', 0):.1f}%"],
    ]
    t = Table(metrics_data, colWidths=[2*inch, 2.5*inch, 1.5*inch])
    t.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), colors.grey),
        ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
        ('GRID', (0, 0), (-1, -1), 0.5, colors.black),
    ...
    params_text = f"Revenue Growth: {params.get('revenue_growth', 0)}% | Churn: {params.get('churn_rate', 0)}% | CAC Change: {params.get('cac_change', 0)}%"
    story.append(Paragraph(params_text, styles['Normal']))
    story.append(Spacer(1, 0.2*inch))
    story.append(Paragraph("Generated by Echolon AI What-If Planner.", styles['Normal']))
    doc.build(story)
    return buffer.getvalue()
