...
# Page configuration
st.set_page_config(page_title="Cohort Analysis - Echolon AI", page_icon="👥", layout="wide")
...
if 'demo_data' in st.session_state:
    ...
    if not cohort_df.empty:
        ...
        with col1:
            st.metric(
                "Avg Cohort Size",
                f"{metrics.get('avg_cohort_size', 0):.0f}",
        ...
        with col2:
            st.metric(
                "6-Month Retention",
                f"{metrics.get('month_6_retention', 0):.1f}%",
                f"-{metrics.get('retention_drop', 0):.1f}%",
                delta_color="inverse"
            )
        ...
        with col3:
            st.metric(
                ...
                f"{metrics.get('best_cohort_retention', 0):.1f}% retention"
            )
        
        with col4:
            st.metric(
                "Total Cohort Revenue",
                f"${metrics.get('total_cohort_revenue', 0):,.0f}",
